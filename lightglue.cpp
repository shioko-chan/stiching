...
LightGlue::LightGlue(std::string modelPath, Stitcher::Mode mode, float matchThresh)
{
	...
	MatchesInfo& matches_info)
{
	...
	Ort::Session lightglueSession(env, this->m_modelPath.c_str(), sessionOptions);
	...
	cout << features1.img_size.width << "\t" << features1.img_size.height << endl;
	...
	if(matches_info.matches.size() < 20){
		return;
	}
	...
	{	
		for (size_t i = 0; i < matches_info.matches.size(); ++i)
		{
			...
			// cout << features1.keypoints[matches_info.matches[i].queryIdx].pt.x << '\t' << features1.keypoints[matches_info.matches[i].queryIdx].pt.y 
			//      << '\t' << features2.keypoints[matches_info.matches[i].trainIdx].pt.x << "\t" << features2.keypoints[matches_info.matches[i].trainIdx].pt.y 
			// 	 << endl;
		}
	...
	else if (this->m_mode == Stitcher::PANORAMA)//perspective mode
	{
		for (size_t i = 0; i < matches_info.matches.size(); ++i)
		{
...
void LightGlue::doMatch(const ImageFeatures& features1, const ImageFeatures& features2,
	MatchesInfo& matches_info)
{
	Ort::Env env(ORT_LOGGING_LEVEL_FATAL, "LightGlue");
	Ort::SessionOptions sessionOptions;
